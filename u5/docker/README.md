...
* `cat /proc/sys/net/ipv4/ip_forward` para consultar el estado de IP_FORWARD (desactivado=0, activo=1). Para activarlo podemos hacerlo de diversas formas:
    ...
    * También podemos crear el fichero `/etc/sysctl.d/alvarez14.conf` y poner dentro lo siguiente:
    ```
...
* `docker commit app1debian diego/nginx1`, a partir del CONTAINERID vamos a crear la nueva imagen que se llamará "nombre-del-alumno/nginx1".
...
* `docker save -o diego14docker.tar nombre-alumno/nginx1`, guardamos la imagen
